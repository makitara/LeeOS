...
const REQUEST_TIMEOUT_MS: Partial<Record<LeeOSMethod, number>> = {
  ...
  [LEEOS_METHOD.systemOpenExternal]: 5_000,
...
const findPluginFrameBySource = (
  source: MessageEventSource | null,
  frameLookupCache: WeakMap<object, PluginFrameInfo>,
) => {
  if (!source || typeof source !== 'object') {
    return null
  }
  const cached = frameLookupCache.get(source)
  if (cached) {
    ...
    frameLookupCache.delete(source)
  }
  ...
  for (const iframe of frames) {
    if (iframe.contentWindow === source) {
      ...
      if (pluginId && origin) {
        ...
        frameLookupCache.set(source, frameInfo)
        return frameInfo
      }
      return null
    }
...
const handlePluginRequest = async (payload: PluginRequestPayload) => {
  ...
  switch (payload.method) {
    ...
    case LEEOS_METHOD.systemOpenExternal: {
      const url = typeof params.url === 'string' ? params.url : ''
      if (!url) {
        return { ok: false, error: 'Invalid params', code: 'ERR_REQUEST_FAILED' }
      }
      return withTimeout(
        window.LeeOS.system.openExternal(url).then((data) => ({ ok: true, data })),
        requestTimeoutMs,
      )
    }
    default:
...
function usePluginBridge() {
  ...
  useEffect(() => {
    const handler = (event: MessageEvent) => {
      ...
      const frame = findPluginFrameBySource(event.source, frameLookupCache.current)
      if (!frame) {
        return
      }
...
export default usePluginBridge
